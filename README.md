...
- **Dashboard rápido: busca em paralelo + cache curto por moeda.** As duas rotas acima batiam na PTAX uma moeda de cada vez, em sequência (a espera de rede é o que pesa, não o processamento) — carregar a tela chegava a levar dezenas de segundos. `_fechamentos_do_recorte` busca as ~15 moedas em paralelo (`ThreadPoolExecutor`, já que `requests` é síncrono) e guarda o resultado de cada uma num cache em memória de 15 minutos, por `(moeda, data_inicio, data_fim)`. Como as duas rotas consultam o mesmo recorte e (na maioria das vezes) o mesmo período, a segunda a carregar reaproveita o cache da primeira. Um TTL curto é seguro aqui porque a PTAX só publica no máximo um boletim de fechamento por dia — nunca serve dado desatualizado dentro da janela.
